#!/usr/bin/env python3
"""
levity-voiced — the shared Levity voice daemon.
...
Coordination policy (finalized — see multi-host/README.md):
  - speak  : INTERRUPT / newest-wins, serialized by _speak_lock (one voice at a
             time, no overlap).
  - capture: single mic lock (_capture_lock); a second listen/confirm while one
             host holds the mic is refused rather than queued.
  - lifecycle: first shim to need the daemon spawns it (detached, start_new_session),
             so it outlives any single host; stale sockets are reclaimed on start.
"""
...
# Serializes speak dispatch so "kill in-flight + start new" is atomic — without
# this, two hosts speaking at once could both kill, then both start, and play
# overlapping audio. Held only for the (synchronous) kill + thread spawn, never
# for playback, so it never blocks the audio itself.
_speak_lock = threading.Lock()


def _dispatch(req: dict) -> dict:
    engine._reload_config_if_changed()
    ...
    if op == "speak":
        ...
        if not snap.get("server_active", False):
            return {"ok": True, "result": "Server is stopped."}
        ...
        # Speak policy: INTERRUPT / newest-wins (finalized). When a second host
        # speaks while another is mid-utterance, the newer message wins and the
        # old one is cut off — this matches single-host behavior and guarantees
        # one voice at a time (no garbled overlap). A queue was considered but
...
def main() -> int:
    ...
    try:
        ...
        # Another daemon won the race between our probe and bind().
        try:
            test = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            test.settimeout(1)
            test.connect(str(SOCKET_PATH))
            test.close()
            sys.stderr.write("[levity-voiced] another daemon won the race; exiting\n")
            return 0
        except OSError:
